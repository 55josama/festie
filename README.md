...
매 시간 성실하게 주간 계획과 회고, 데일리 스크럼에 참여하며 체계적인 개발 과정을 이어갈 수 있었습니다.  
구현 과정에서 고민이 생길 때마다 팀원들과 적극적으로 의견을 주고받으며, 보다 완성도 높은 프로젝트를 구현할 수 있었던 점이 좋았습니다.

### 이수빈

데일리 스크럼과 빠른 소통을 바탕으로 팀워크를 유지할 수 있었고, 부하 테스트 기반 성능 개선을 경험하며 기술적으로도 성장할 수 있었습니다.  
다만 코드 리뷰를 CodeRabbit에 의존한 점, 채팅 기능 외 추가적인 도전 아이디어가 부족했던 점, 동시성 문제를 직접 깊게 경험하지 못한 점은 아쉬웠습니다.

### 김란미

모르는 부분이 생겼을 때 팀원들과 함께 공유하고 이야기 나누며 해결해 나갈 수 있어서 좋았습니다.  
다만 GitHub 리뷰 과정에서 CodeRabbit에 의존하게 된 점은 아쉬웠습니다.

### 김현희

회의를 통해 각자의 진행 상황과 이슈를 파악할 수 있었고, 이를 바탕으로 프로젝트를 무사히 마무리할 수 있어서 좋았습니다.  
다만 맡은 기능을 끝까지 충분히 책임지지 못한 점은 아쉬움으로 남았습니다.

### 이영재

서로 배려하며 의견 충돌이나 문제 발생 시 원만하게 해결한 점이 좋았습니다.  
다만 초기 구조 설계가 부족해 개발 후반부에 수정이 발생한 점은 아쉬웠습니다.

### 백가은

Grafana 대시보드를 활용하며 수치 변화를 확인하고 분석한 점이 흥미로웠습니다.  
다만 부분 인덱스 개선에 따른 부하 테스트를 동일한 환경에서 정확히 모니터링하지 못했고, PostgreSQL shared buffer 캐시 상태와 autovacuum 타이밍을 미리 고려하지 못한 점은 아쉬웠습니다.
