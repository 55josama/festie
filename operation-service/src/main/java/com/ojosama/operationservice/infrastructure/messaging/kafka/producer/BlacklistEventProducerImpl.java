...
public class BlacklistEventProducerImpl implements BlacklistEventProducer {
    ...
    public void publishStatusChangeEvent(UserBlacklistStatusEvent event) {
        try {
            ...
            // 타임아웃(3초) 설정
            kafkaTemplate.send(statusTopic, event).get(3, TimeUnit.SECONDS);
            log.info("Kafka 이벤트 발행 성공 - topic: {}, userId: {}", statusTopic, event.getUserId());

        } catch (Exception e) {
            log.error("Kafka 이벤트 발행 실패. DB 트랜잭션을 롤백합니다. topic: {}, event: {}", statusTopic, event, e);
            throw new IllegalStateException("이벤트 발행에 실패하여 블랙리스트 처리를 취소합니다.", e);
        }
    }
}
