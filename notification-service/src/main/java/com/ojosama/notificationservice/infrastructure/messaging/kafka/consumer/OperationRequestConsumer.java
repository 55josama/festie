...
public class OperationRequestConsumer {
    ...
    public void onMessage(ConsumerRecord<String, String> record) {
        ...
        try {
            ...
            log.info("운영 요청 이벤트 성공 : {}", record.key());
    ...
    private void dispatch(OperationRequestMessage event) {
        if (event == null) {
            throw new NotificationException(NotificationErrorCode.INVALID_MESSAGE_PAYLOAD);
        }
        notificationService.operationRequest(
                new OperationRequestCommand(event.requestId(), event.requesterId(), event.title()));
    }
}
