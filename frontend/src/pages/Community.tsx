      ...
      <section className="rounded-[24px] border border-[var(--line)] bg-white p-3 shadow-[0_12px_30px_rgba(15,23,42,0.04)] md:p-4">
        <div className="flex flex-col gap-2.5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <FilterChip active={!categoryId} tone="violet" onClick={() => setCategoryId(undefined)}>전체</FilterChip>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <FilterChip active={sort === 'latest'} tone="sky" onClick={() => setSort('latest')}>최신글</FilterChip>
            <FilterChip active={sort === 'popular'} tone="sky" onClick={() => setSort('popular')}>인기글</FilterChip>
          </div>
        </div>
      </section>
...
function FilterChip({ active, tone = 'violet', onClick, children }: { active?: boolean; tone?: 'violet' | 'sky' | 'emerald' | 'rose' | 'slate'; onClick: () => void; children: ReactNode }) {
  const toneClass = {
